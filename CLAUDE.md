...
7. **DB 적재** — ⚠️ **판정 승자는 `qwen3-embedding-0.6b` 지만 7~9단계 첫 관통은 기준선 `bge-m3` 로 간다**
   (사용자 결정 2026-08-24, D-024 `판정 이후` 절). 관통에서 재는 것은 검색 품질이 아니라 배선이고,
   Qwen3 는 비대칭이라 문제가 나면 원인이 둘이 된다. **적재기·검색기는 모델을 인자로 받고**,
   `metadata.embedding_model` 에는 **실제로 쓴 모델**을 적는다. 9단계까지 닿으면 승자로 바꿔 재적재
