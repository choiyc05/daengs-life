...
> ⚠️ **이관 예정 (2026-08-27)** — 이 레포는 팀 레포 **`SAJOYO/DAENGS_dev`** 로 들어간다. 2랩부터는 저쪽에서 저쪽 규칙(`dev` 브랜치 · PR 본문 `## 작업 목록`)으로 진행한다.
> 옮길 때 부딪히는 것 11개(**ADR 접두사 충돌 → `RAG-` 로 개명 완료** · src 레이아웃 · SQLAlchemy vs psycopg3 · redis 없음 · `dev` push = 자동 배포)와
> **git 이 안 들고 가는 것**(특히 재현 불가능한 `data/processed/answers/lap1.jsonl`)은 **`docs/handoff-daengs-dev.md`** 에 있다.

## 작업 방식 (중요)
- 파트별 **논의 → 결정 기록(ADR, 아래 번호 규칙) → 구현** 순서. 사용자가 명시하기 전에 구현으로 넘어가지 말 것
- 결정 상태: ✅ 확정 / 🔶 제안(논의중). 논의 결과는 결정 파일에 반영하고 커밋
- **ADR 번호는 파트별로 파일이 갈린다** — 공통·파트① = `docs/decisions-rag.md` 의 `RAG-` /
...
- 크롤링 산출물은 `data/` (규칙: `data/README.md` — 원본 불변, `.meta.json` 필수, meta 없으면 인덱싱 금지,
  **`data/` 의 수집 결과물은 전부 git 미추적** — 원본·meta·크롤로그 모두 로컬 상태다 (RAG-017).
...
## 스택
- backend: FastAPI + uv (Python), LLM은 Gemini API 예정. **`backend/` 는 uv 프로젝트 하나**이고 그 안에 패키지가 다섯으로 나뉜다 (RAG-018 · RAG-027): `crawler/` 수집 · `rag/` 인덱싱(parse→chunk→embed→load→search) · `realtime/` 실시간 · `tasks/` Celery 워커·Beat · `app/` 서빙(controllers·services·dto). **`rag/` 안은 순서로 가른다**(RAG-023) — `pipeline.py`(순서의 단일 소스) · `core/`(config·io·ir) · `stages/`. 의존 방향은 **app→{rag,realtime}→crawler** 한쪽뿐이고 `tests/test_import_direction*.py` 둘이 막는다 (방향 + **범위**).
  실행: `cd backend && uv run python -m crawler run --source <id>` / `uv run python -m rag parse`
- 로컬 인프라: `docker compose up -d` — `db`(postgres+pgvector) · **`redis`**(RAG-001 브로커·캐시, RT-001 ④ 가 첫 사용처)
...
- DB: postgres + **pgvector 0.8.6** — `documents` 테이블: `embedding vector(1024)`, `content_hash`(중복 방지 자연키), category CHECK(policy/travel/food), source_type CHECK(document/web/api/manual), source_url/document_title/section 컬럼 (조항 인용용) — RAG-008
- 오케스트레이션: **Celery + Beat + Redis** (RAG-001 확정 — Airflow 아님)
...
- **수집 28건 — §1 공통·법령 파트 전체 완료**
  - `easylaw-pet` 14 (해설 + 100문100답 → RAG-007 골든셋 재료)
...
- 확정: RAG-001 오케스트레이션 / RAG-008 저장 규약·스키마 / RAG-009 크롤러 배치·소스 계약 / RAG-010 원본 부재 재수집 /
  RAG-011 법령 웹 원문 경로 / RAG-012 소스 도메인 분류·API 키 규칙 / RAG-013 MCP 미채택 / RAG-014 env 배치·의존방향 가드 /
  RAG-015 pydantic-settings / RAG-016 법령 API 수집 완료 / **RAG-004 청킹 전략(구조 기반)** /
  **RAG-018 인덱싱 패키지 경계(`backend/rag/`)·파서 3층·공통 IR 6종** / **RAG-019 parsed 산출물 규약** / **RAG-020 별표 표 파싱** /
  **RAG-021 청커 5결정(입력범위·길이기준·content조립·폴백없음·산출물규약)** /
  **RAG-022 골든셋 6결정(필수·보강 2층·법제처 라벨 차용·15문항·날짜 뺀 논리 주소)** /
  **RAG-023 `rag/` 배치(`core/`+`stages/`+`pipeline.py`)** /
  **RAG-024 3파전 4결정(사전등록·기준선 우선 / 판정 k=5 / 탈락선 Hit@5 2문항 차 · Recall·MRR 은 기록만 / 덤프 미추적·판정은 ADR)**
  / **RAG-025 7단계 적재 5결정(upsert+한 트랜잭션 · psycopg3+SQL 직접 · 중복은 대표+merged_from · source 를 파서·청커에 실음 · 임베딩 지문을 (chunk_id,content) 로)**
  / **RAG-026 8단계 검색(부칙 기본 포함·필터는 플래그 / 검색은 `stages/search.py` 하나, CLI·9단계·FastAPI 가 같은 함수)**
    — **판정 완료: 승자 `Qwen/Qwen3-Embedding-0.6B`** (Hit@5 6:4:3, 나머지 둘 탈락). **첫 관통은 `bge-m3` 로**
  / **RAG-027 서빙 계층 배치**(파트②가 발급, **두 파트 공통**) — `app/`={controllers,services,dto,deps}, 강제 규칙은 *컨트롤러에 로직 0줄*
  / **RAG-028 9단계 서빙**(모델 수명은 호출자 소유·서빙은 CPU 상주 / `/ask` 는 답변+근거 — 1랩 응답은 제품 계약 아닌 관찰 도구 /
  `def` 컨트롤러 / 커넥션은 요청당·`deps.py` 경유 / 답변 덤프는 결정적 축 3개·논리 주소 병기)
  — **③: 배치는 RAG-027 을 따르되 조립 순서(`검색→생성`)는 `rag.stages.generate.ask()` 가 소유한다.**
  파트②가 `services/walk.py` 에 조립을 둔 것과 갈리는 이유는 **검문소가 사는 층이 달라서**다 —
  파트② 검문소는 전부 API 레벨, 파트① 검문소③④는 CLI 이고 `rag` 는 FastAPI 없이 돌아야 한다(RAG-001 원칙 1)
- 논의중(🔶):
  RAG-003 하이브리드·리랭커, RAG-005 실행순서, RAG-006 EDA 시스템화, RAG-007 평가 체계
- **다음 목표: 수집한 28건 전부로 RAG 한 세트 관통** (아래 9단계). 소스 추가는 그 뒤로 미룸
- ⚠️ `data/` 는 미추적(RAG-017)이라 **PC 마다 상태가 다르다.** 새 PC 에서는 `backend/.env` 에 LAW_OC 를 넣고
...
### RAG 관통 실행 순서
1. ~~**RAG-004 확정**~~ ✅ 2026-08-20 — 질문 1~7 정답 위치를 원문에서 확인, 구조 기반 전략 + 판정표 확정
...
3. ~~**청커**~~ ✅ 2026-08-24 — RAG-021 ①~⑤ 확정 + `rag/stages/chunk.py` 구현. `processed/chunks/*.jsonl` **1,407청크**
...
5. ~~**골든셋 시드**~~ ✅ 2026-08-24 — RAG-022 ①~⑥ 확정 + `rag/stages/goldenset.yaml` 구현.
...
6. ~~**3파전 비교**~~ ✅ 2026-08-24 — RAG-024 ①~④ 확정 + `rag/stages/evaluate.py` 구현.
   **승자 `qwen3-embedding-0.6b`** (Hit@5 **6/15** : kure 4 : bge 3 — 나머지 둘이 2문항 이상 낮아 탈락).
   ...
   → **RAG-003(하이브리드·리랭커)이 필요하다는 실측 근거**. 절대 판정은 8단계 검문소③이 한다
   ⚠️ **RAG-022 ④ 가설이 뒤집혔다** — `제2조` 는 공짜 점수가 아니라 **세 모델 다 top-5 진입 0/6**(중앙 116~442위)인
   닿지 못하는 라벨이다. `nice` 강등 조건은 충족됐고 판정은 그와 무관하게 성립한다(Hit 은 안 바뀐다). Q4 는 제외 대상
7. ~~**DB 적재**~~ ✅ 2026-08-25 — RAG-025 ①~⑤ 확정 + `rag/stages/load.py` 구현. **`documents` 1,402행**
   (1,407청크 − content 중복 5건, `metadata.merged_from` 에 보존) · `embedding_model = BAAI/bge-m3`.
   ⚠️ **판정 승자는 `qwen3-embedding-0.6b` 지만 첫 관통은 기준선으로 간다** (RAG-024 `판정 이후`).
   교체는 `--model` 한 번이고 실제로 양방향 확인했다. 적재 벡터 = parquet (오차 0.0)
   ⚠️ **교체 조건은 1랩 완료로 발동했으나 2랩으로 미뤘다** (2026-08-25, 사용자 — RAG-024 `교체 조건 발동`).
   1랩 덤프가 RAG-028 ⑥의 비교 축이라 지금 바꾸면 1랩↔2랩 차이의 원인이 코퍼스인지 모델인지 안 갈린다.
   **2랩은 코퍼스와 모델이 같이 바뀌는 랩**이 된다 — 그때 원인을 하나로 특정할 수 없음을 명시할 것
   **RAG-025 ⑤ — 임베딩 지문을 `(chunk_id, content)` 로 좁혔다.** 메타 필드를 더해도 재임베딩이 안 걸린다:
...
8. ~~**검색 CLI (dense만)**~~ ✅ 2026-08-25 — RAG-026 확정 + `rag/stages/search.py`. **검문소③ 통과** —
   필수가 top-5 에 든 문항 **2/7**(Q5·Q7)이고 6단계 `bge-m3` 결과와 정합(적재가 벡터를 옳게 옮겼다는 독립 확인).
   부칙은 **기본 포함**, `--no-supplementary` 로 끈다 — 검사 도구와 서빙의 기본값은 다르다(RAG-026 ①). 켜도 결과 동일
   **검문소③이 잡은 것 둘**: ⓐ dense 가 질문의 *의도* 가 아니라 *표면 어휘* 를 따라간다
   (Q1 처벌→'등록' 해설만 · Q3 목줄→가축전염병 과태료 별표 · Q6 광견병→혈청검사 수수료) = **RAG-003 의 근거**
   ⓑ **골든셋이 Hit 을 과소평가한다** — easylaw 해설이 본문에 조항 번호를 담고 있어 실제로는 답인데 `must` 가 아니다.
   즉 `Hit@5=0.200` 은 "20%만 답한다"가 아니라 "법령 조문에 20%만 닿는다" (RAG-022 재검토 대기)
...
9. **LLM 생성**(Gemini) + FastAPI `/ask` → (선택) Next.js  ← ✅ **1랩 완료 2026-08-25. 검문소④ 통과** (PR #3, 8/27 머지)
   **한 랩 = `/ask` 까지**(사용자, 2026-08-25). 그 뒤 **1단계 소스를 최대한 확장해 2랩**을 돈다 —
   그래서 1랩의 목표는 좋은 답이 아니라 **2랩과 비교 가능한 상태**다(RAG-028 ⑥ 답변 덤프)
   `app/` 배치는 **RAG-027**(controllers/services/dto/deps)을 따른다. 다만 **조립은 `rag.stages.generate.ask()` 가 소유한다** —
   `rag` 는 `app` 을 import 할 수 없어(RAG-014) 조립이 `services/ask.py` 에 있으면 **검문소④가 서빙과 다른 코드를 검사**하게 된다(RAG-026 ② 위반).
   `services/ask.py` 가 담는 것: **서빙 정책 기본값**(k·부칙 — RAG-026 ①이 비워 둔 자리) · 에러 매핑 · DTO. **생성은 한 줄도 없다**
   ...
   **방침(사용자, 2026-08-25): 먼저 한 세트 붙여서 돌려보고 판단한다.** 설계 논의를 앞세우지 말 것 —
   답이 실제로 어떻게 나오는지 보고 나서 프롬프트·거부 전략을 정한다(RAG-029 자리)
   ...
   `app/main.py` 등록 한 줄(RAG-027 이 정한 자리) · 테스트 2벌. `backend/main.py` shim 정리는 그다음
   ...
   `GEMINI_API_KEY` 는 `backend/.env` 에 **이미 있다**. 검색은 `rag.stages.search.search()` 를 그대로 부른다(RAG-026 ②)
   실행: `uv run fastapi dev app/main.py` → `POST /ask {"question": "..."}` · 검문소④는 `uv run python -m rag generate --questions`
   ⚠️ **`fastapi dev` 는 reload 가 기본**인데 lifespan 이 임베딩 모델을 올린다(RAG-028 ①) — 저장할 때마다 로드 5~7초를 다시 문다.
...
- 필요 키(**`backend/.env`**): LAW_OC ✅ / DATA_GO_KR_KEY ✅ / KAKAO_REST_KEY ✅ / KMA_HUB_KEY ✅ (2026-08-24 전부 발급) — 발급처는 docs/data-sources.md §9, 함정은 아래 파트② 절
  - env 배치는 배포 단위 기준(RAG-014): 루트 `.env`=인프라(compose), `backend/.env`=백엔드 런타임, `frontend/.env`=프론트
...
- **서빙 계층 배치는 RAG-027** (공통 결정이라 `RT-` 가 아니다) — MVC2 를 이 레포 사실에 맞춘 것.
  `app/`={controllers,services,dto,deps} · M 은 `realtime/`·`rag/` 그대로 · **`models/` 는 없다**(근거는 RAG-027)
