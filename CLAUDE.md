...
- 논의중(🔶):
  **D-027 9단계 서빙** — ①~⑤ 확정(2026-08-25): 모델 수명은 **호출자 소유**(`encode(st=...)` 는 받으면 안 내린다,
  `search(conn=...)` 와 같은 패턴) · **서빙은 CPU 에 한 벌 상주(lifespan), 배치(embed/evaluate)는 GPU**.
  근거는 실측 — device 를 바꿔도 검증질문 7개의 top-5 가 **순서까지 동일**하고 검문소③ 2/7 이 재현된다
  (cos≥0.99999994, 인코딩 cuda 51ms : cpu 124ms, VRAM 2.3GB).
  ② `/ask` 는 **답변 + 근거**를 함께 돌려준다(`content` 전문). 1랩 응답은 **관찰 도구지 제품 계약이 아니다** —
  검문소④가 "인용한 조항이 컨텍스트에 실재하나"를 세려면 본문이 필요하다. 매핑은 `app/schemas.py` 가 하고 `rag` 는 pydantic 을 모른다.
  ③ `app/` 은 층을 안 만든다 — service 자리는 이미 `rag/` 다. **조립 순서(`검색→생성`)는 `rag.stages.generate.ask()` 에
  한 번만 적고** CLI·라우터가 그걸 부른다(D-003 이 8·9단계 사이에 끼면 고칠 곳이 하나여야 한다).
  `backend/main.py` → `app/{main,schemas}.py` + `app/routers/ask.py`, 실행은 `fastapi dev app/main.py`.
  `routers/` 를 지금 두는 건 `feat/realtime` 이 곧 `/walk` 를 같은 `app/` 에 붙이기 때문(머지 충돌을 한 줄로 줄인다).
  ④ `def`(스레드풀) — sync 코드를 `async def` 에 넣으면 루프가 멈춘다 · ⑤ 커넥션은 요청당.
  ⑥(1랩 답변 덤프)만 미정. 거부 전략은 한 랩 돌린 뒤(D-028 자리)
...
9. **LLM 생성**(Gemini) → 이후 FastAPI `/ask` → (선택) Next.js  ← **다음 세션 여기서 시작**
   **방침(사용자, 2026-08-25): 먼저 한 세트 붙여서 돌려보고 판단한다.** 설계 논의를 앞세우지 말 것 —
   답이 실제로 어떻게 나오는지 보고 나서 프롬프트·거부 전략을 정한다
