"""7단계 적재 — `chunks/` + `embeddings/{key}.parquet` → `documents` (D-008, D-025).

**테이블은 이미 D-008 이 만들었다**(`db/init/01_schema.sql`). 이 단계는 그 테이블을 채운다.
...
① **`ON CONFLICT DO UPDATE` + 전체 한 트랜잭션.** 스키마 주석이 적어 둔 `DO NOTHING` 은 모델을
   ...
   같고, `DO NOTHING` 이면 **1,407행 전부를 조용히 건너뛴 채 옛 벡터가 남는다.** 그러면 D-024
   `판정 이후` 가 세운 "교체는 같은 명령 재실행"이 거짓이 된다.
② **한 트랜잭션인 이유는 원자성이 곧 D-002 의 집행 장치**라서다. upsert 는 행 단위 갱신이라
   중간에 죽으면 `embedding` 한 컬럼에 두 모델의 벡터가 섞인다 — D-002 가 금지한 그 상태를
   중간 결과로 만든다. 1,407행이면 한 트랜잭션이 부담도 아니다.

**적재 모델은 인자다.** 지금 기본값은 판정 승자(`qwen3-embedding-0.6b`)가 아니라 기준선
`bge-m3` 인데, 그것이 D-024 `판정 이후` 의 결정이다 — 첫 관통에서 재는 것은 검색 품질이 아니라
...
# 컬럼으로 가는 것과 metadata 로 가는 것 (D-008 표준 키 + D-025 ④).
...
def prepare(model_key: str) -> Prepared:
    ...
    **중복 처리 (D-025 ③)** — `content_hash` 가 같은 청크는 한 행으로 합친다. 실물 5쌍은 전부
    ...
    for chunk, vector in zip(chunks, vectors):
        ...
        meta["embedding_model"] = model.repo      # **실제로 쓴 모델** (D-024 판정 이후)
...
# ---------------------------------------------------------------- DB
def connect():
    """psycopg3 연결 + pgvector 어댑터 등록 (D-025 ②).
...
def existing_models(conn) -> list[tuple[str, int]]:
    """지금 테이블에 들어 있는 (embedding_model, 행 수). 적재 전에 보여 준다 (D-025 ①).
