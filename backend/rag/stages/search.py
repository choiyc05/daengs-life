"""8단계 검색 — `documents` 에서 dense 로 top-k 를 꺼낸다 (D-026).

**9단계(Gemini)를 붙이기 전에 검색만 따로 떼어 둔다.** 답변이 이상할 때 검색이 틀렸는지 LLM 이
틀렸는지 가르려면 검색이 단독으로 관찰 가능해야 한다 — D-024 `판정 이후` 가 기준선으로 관통하기로
한 것과 같은 판단이고, 원인을 하나로 두려는 것이다.

**여기가 검색의 유일한 구현이다** (D-026 ②). CLI 는 이 결과를 찍기만 하고, 9단계와 FastAPI 도
같은 함수를 부른다. 경계를 안 지키면 9단계가 검색을 다시 짜게 되고, 그 순간 **검문소③이 확인한
것과 서빙이 실제로 하는 것이 달라진다** — 검문소③의 근거가 조용히 무효가 되는 자리다.

D-003(하이브리드·리랭커)이 들어와도 **바뀌는 것은 이 파일의 내부이지 시그니처가 아니다.**
...
# 검문소③이 보는 수이자 D-024 ② 의 판정 k. **같은 숫자여야 한다** — 평가에서 쓴 k 와 눈으로 보는
...
class Hit:
    ...
    "이게 정답 청크인가"를 눈으로 대조할 수 있다 (D-022 ⑥B 의 논리 주소)."""
    rank: int
    ...
    document_title: str
    content: str
    part: str | None              # "supplementary" = 부칙 (D-021 ①)
...
def encode(query: str, model_key: str | None = None):
    ...
    승자가 아니라 기준선 `bge-m3` 이고, 그것이 D-024 `판정 이후` 의 결정이다.
    """
...
def search(query_vector, *, k: int = DEFAULT_K, include_supplementary: bool = True,
    ...
    **`include_supplementary` 기본이 `True` 인 것은 결정이다** (D-026 ①). 부칙은 세 모델 모두에서
...
def hand_questions() -> list[tuple[str, str, set[str], set[str]]]:
    ...
    **질문을 코드에 박지 않고 `goldenset.yaml` 에서 읽는다** (D-026 ②). 박으면 질문 목록의 단일
    소스가 둘이 되고, D-022 가 골든셋을 git 에 넣은 이유(판단 기록의 단일 소스)가 반쯤 무너진다.
...
def tier_of(chunk_id: str, must: set[str], nice: set[str]) -> str:
    """검색 결과의 `chunk_id` 가 정답 라벨인가. 라벨은 **수집 날짜를 뺀 논리 주소**다 (D-022 ⑥B)."""
    from . import goldenset
