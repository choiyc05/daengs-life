...
문장과 금액이 엉키고 `제101조제4항제4호` 라는 인용 문자열을 복원할 수 없다 (D-020 의 B안).
...
# (원본이 셀 폭에 맞춰 임의 위치에서 끊으므로 공백을 넣으면 `위반하 여` 가 된다 — D-020)
...
def parse_table(tlines: list[str]) -> TableBlock:
    ...
    flush()
    # 헤더의 물리적 줄은 의미 단위(`1차` / `위반`)라 본문과 규칙이 다르다 — 공백으로 잇는다 (D-020)
