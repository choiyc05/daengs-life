"""`POST /ask` 의 공개 계약 (D-028 ② · D-027 의 V 층).

⚠️ **1랩의 이 응답은 제품 계약이 아니라 관찰 도구다** (D-028 ②). 검문소④가 읽는 것 —
...
`dto/walk.py` 와 같다 — D-003(하이브리드·리랭커)이 `Hit` 에 필드를 더하는 것은 도메인의 자유여야
...
class AskIn(BaseModel):
    ...
    # 갖는다 — D-026 ①이 *"검사 도구와 서빙이 같은 기본값을 쓸 이유가 없다"* 며 비워 둔 자리다.
