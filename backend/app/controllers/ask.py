"""`POST /ask` (D-028 · D-027 의 C 층).

**로직이 없다.** 질문을 받고, 서비스를 부르고, 그대로 돌려준다. 검색도 프롬프트도 판정도 여기
없다 — 그것이 D-027 의 유일한 강제 규칙이고, `controllers/walk.py` 가 세워 둔 본보기 그대로다.

`def` 인 것은 결정이다 (D-028 ④) — psycopg3 도 임베딩 인코딩도 sync 라 `async def` 안에 넣으면
