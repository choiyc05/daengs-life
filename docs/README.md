...
| [decisions-realtime.md](decisions-realtime.md) | **파트② 설계 결정 기록 (`RT-`)** — RT-001 실시간 엔진 6결정(①계층 ✅확정: 전송 층 `transport/` 3 · provider 층 `providers/` 7 분리, `realtime`→`crawler.core.config` 참조 허용 + 가드 2건 — **API허브가 두 계열이라는 실측 정정 붙음** / ②~⑥ 미논의, ②는 실측 확보 완료). **번호를 `D-` 와 나눈 이유는 그 파일 헤더에** |
