...
| [decisions-realtime.md](decisions-realtime.md) | **파트② 설계 결정 기록 (`RT-`)** — RT-001 실시간 엔진 6결정(①계층 ✅확정: 전송 층 `transport/` 3 · provider 층 `providers/` 7 분리, `realtime`→`crawler.core.config` 참조 허용 + 가드 2건 — **API허브가 두 계열이라는 실측 정정 붙음** / **②공통 관측 모델 ✅확정**: 관측값·상태·위치 해석 3분할, `Measurement` 목록, `valid_at`+`issued_at`, `value = float|Code|Interval`, `Q` 23개, 묶음 타입 `Observations` / ③~⑥ 미논의). **번호를 `D-` 와 나눈 이유는 그 파일 헤더에** |
