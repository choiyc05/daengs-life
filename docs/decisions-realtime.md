...
| ②-d | 어휘 — 양(quantity)의 닫힌 목록 · 등급은 provider 가 주나 룰이 만드나 | ⬜ | 형태가 정해진 뒤 그 안을 채운다 |
...
> `observed_at` 은 ②-c 에서 `valid_at` + `issued_at` 으로 **세분됐다** (모순이 아니라 쪼갠 것이므로
> ②-b 의 결정은 그대로 유효하다). 최종 필드는 ②-c 를 볼 것.
...
**②-d 어휘** — `Q` 에 무엇을 넣나, 그리고 **등급을 provider 가 주나 룰이 만드나**. 실측이 던진
구체적 질문 둘: ① 에어코리아는 등급을 이미 주는데(`pm10Grade1h`) 기상청은 안 준다 — 비대칭을
provider 층에서 없앨 것인가 ② `PTY`(강수형태 0/1/2/3/4)·`SKY` 같은 **코드 값**은 `value: float` 에
안 맞는다.
