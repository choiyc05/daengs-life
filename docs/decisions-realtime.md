...
| ②-c | 시간축 — 예보는 관측과 같은 타입인가 | ⬜ | ②-b 가 정해져야 "같은 타입"이 뭔지 말이 된다 |
...
**②-c 시간축** — 예보를 관측과 같은 타입으로 담나. 실측이 이미 한쪽으로 밀고 있다: 오늘 폭염 판정에
쓰인 33℃ 는 **실황이 아니라 예보값**(`getVilageFcst` `TMP` 15:00)이었고, 모든 provider 가
**발표 시각과 대상 시각을 따로** 준다 (`baseTime`/`fcstTime`, 특보 `tmFc`/`tmEf`,
예보통보 `dataTime`/`informData`). `Measurement` 에 시각이 하나로 충분한지가 질문이다.
