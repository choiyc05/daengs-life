...
| ②-b | **형태** — 필드 고정 스냅샷이냐 측정값 목록이냐 | 🔶 논의중 | 본체 |
...
**②-b 형태** — 관측값을 필드 고정 스냅샷으로 담나, 측정값 목록으로 담나. 판정 기준은 §6.3 의
모양 A~E 를 각 후보로 표현해 보고 ① 룰 코드에 API 고유 이름(`category`·`T1H`/`TMP`·`pm10Grade1h`)이
나오는가 ② **값마다 다른 신선도·출처**를 표현할 수 있는가 ③ 부분 실패를 표현할 수 있는가.
