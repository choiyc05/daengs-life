...
### 5) `law-drf-api` 는 코드만 준비하고 실행은 보류

LAW_OC 미발급이라 **실제 응답으로 돌려본 적이 없다.** XML 태그 이름은 법제처 공개 규격 기준이고 파서는 태그 후보를 여러 개 받는 느슨한 구조다. 키 발급 후 `--dry-run` 으로 확인할 항목은 모듈 docstring 에 4개로 적어 두었다 (검색/본문의 법령명 태그 차이, 조문단위 반복 구조, **키 마스킹 육안 확인**, 응답 바이트 안정성).
...
**발급 시 주의 (2026-08-20 실물 확인)** — DRF 는 OC 문자열만으로 되지 않는다. 신청할 때 **호출할 서버의 IP/도메인을 등록**해야 하고, 등록 없이 호출하면 HTTP 200 에 `<Response><result>사용자 정보 검증에 실패하였습니다</result>` XML 이 온다. 인증 실패를 200 으로 주므로 `_check_auth_error()` 가 규격 오류로 오진하기 전에 먼저 걸러낸다. 이 경로는 실제로 확인했고, **정상 응답 경로만 미검증**이다.
