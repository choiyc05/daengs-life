...
| 무엇 | 크기 | 다시 만들 수 있나 |
|---|---|---|
| `data/raw/**` 원본 28건 + `.meta.json` | — | ✅ 재수집 (§4) |
| `data/processed/{parsed,chunks}` 22+22 | — | ✅ 결정적으로 재현된다 (2026-08-22 요소 수 동일 확인) |
| `data/processed/embeddings/*.parquet` 3종 | 6.4MB×3 | ✅ 재현. 단 GPU 로 수십 분 |
...
`data/` 가 미추적이라 PC 마다 상태가 다르다. 아래를 그대로 밟으면 28건/22 parsed 가 복원된다 (2026-08-22 실제로 복원, 요소 수 동일 재현 확인).
